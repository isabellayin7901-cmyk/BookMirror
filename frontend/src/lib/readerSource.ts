 ...
 */
import {
  fetchReaderChapter, fetchReaderToc, type ReaderChapter, type ReaderToc,
} from './api';
import { getDemoBook } from '../data/demoBooks';
import type { BookEdition, ChapterSource, ContentLanguagePref, Language } from '../types';
...
export async function loadReaderBook(bookId: string, editionId: string | null): Promise<{ info: ReaderBookInfo; toc: ReaderToc } | null> {
  ...
  if (demo) {
    ...
    return {
      info: { bookId, title: demo.title, isDemo: true, chapterSource: demo.chapterSource, editions: demo.editions },
  ...
  if (!toc) return null;
  return { info: { bookId, title: toc.title, isDemo: false, editions: [] }, toc };
}

export async function loadChapter(bookId: string, index: number, editionId: string | null): Promise<ReaderChapter | null> {
  const demo = getDemoBook(bookId);
  if (demo) {
    const ed = editionId && demo.chapters[editionId] ? editionId : demo.editions[0].id;
    const chapters = demo.chapters[ed];
